...
static
ARCHI_HASHMAP_TRAV_KV_FUNC(archi_opencl_program_sources_from_hashmap_traverse_func)
{
    archi_opencl_program_sources_t *sources = data;
        ...
        archi_status_t *code)
{
    if (hashmap == NULL)
    {
        ...
        return (archi_opencl_program_sources_t){0};
    }

    archi_opencl_program_sources_t sources = {0};
    ...
    archi_status_t trav_code = archi_hashmap_traverse(hashmap, true,
            archi_opencl_program_sources_from_hashmap_traverse_func, &sources);
    if (trav_code < 0)
    {
        if (code != NULL)
            *code = trav_code;
        ...
        archi_status_t *code)
{
    if (array == NULL)
    {
        ...
        return (archi_opencl_program_binaries_t){0};
    }

    archi_opencl_program_binaries_t binaries = {0};

    binaries.ids = archi_opencl_platform_device_ids_alloc(num_elements, NULL);
        ...
        archi_status_t *code)
{
    ...
    // Get program devices and platform ID
    {
        ...
        binaries.ids = archi_opencl_platform_device_ids_alloc(num_devices, NULL);
        if (binaries.ids == NULL)
        {
            if (code != NULL)
                *code = ARCHI_STATUS_ENOMEMORY;

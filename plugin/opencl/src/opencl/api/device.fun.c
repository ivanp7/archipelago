...
{
    archi_opencl_platform_device_ids_t *ids = malloc(
            ARCHI_SIZEOF_FLEXIBLE(archi_opencl_platform_device_ids_t, device_id, num_devices));
    if (ids == NULL)
        return NULL;
        ...
        archi_status_t *code)
{
    ...
    archi_opencl_platform_device_ids_t *ids =
        archi_opencl_platform_device_ids_alloc(num_devices, NULL);
    if (ids == NULL)
    {
        if (code != NULL)

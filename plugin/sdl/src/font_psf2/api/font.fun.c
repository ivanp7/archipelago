        ...
        archi_status_t *code)
{
    ...
    if (num_bytes < sizeof(archi_font_psf2_header_t))
    {
        if (code != NULL)
    ...
    const archi_font_psf2_header_t *header = bytes;
    if ((header->magic != ARCHI_FONT_PSF2_MAGIC) || (header->version != 0))
    {
        if (code != NULL)
            *code = ARCHI_STATUS_EVALUE;
        return NULL;
    ...
    if ((header->header_size < sizeof(archi_font_psf2_header_t)) ||
    ...
    font->header = (archi_font_psf2_header_t*)header;
    ...
    else
    {
        ...
        {
            uint32_t code_point;
            size_t seq_len = archi_decode_utf8_code_point(table, remaining_bytes, &code_point);
        ...
        size_t *chr_len)
{
    ...
    uint32_t code_point;
    size_t seq_len = archi_decode_utf8_code_point(

/**
 * @file
 * @brief Application context interface for SDL windows (CPU renderer).
 */

#include "archi/sdl2/ctx/window_cpu.var.h"
#include "archi/sdl2/api/window_cpu.fun.h"
#include "archipelago/util/size.fun.h"
#include "archipelago/util/string.fun.h"

#include <stdlib.h> // for malloc(), free()

struct archi_context_sdl2_window_cpu_data {
    archi_pointer_t context;

    struct {
        int width;
        int height;
    } texture;

    struct {
        int pitch;
        int x;
        int y;
        int width;
        int height;
    } texture_lock;
};

ARCHI_CONTEXT_INIT_FUNC(archi_context_sdl2_window_cpu_init)
{
    // Parse parameters
    archi_sdl2_window_cpu_params_t window_params = {0};
    archi_sdl2_window_cpu_params_t window_params_fields = {0};

    bool param_params_set = false;
    bool param_texture_width_set = false;
    bool param_texture_height_set = false;
    bool param_window_width_set = false;
    bool param_window_height_set = false;
    bool param_window_flags_set = false;
    bool param_window_title_set = false;

    for (; params != NULL; params = params->next)
    {
        if (ARCHI_STRING_COMPARE("params", ==, params->key))
        {
            if (param_params_set)
                continue;
            param_params_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, archi_sdl2_window_cpu_params_t))
                return ARCHI_STATUS_EVALUE;

            window_params = *(archi_sdl2_window_cpu_params_t*)params->value.ptr;
        }
        else if (ARCHI_STRING_COMPARE("texture_width", ==, params->key))
        {
            if (param_texture_width_set)
                continue;
            param_texture_width_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, int))
                return ARCHI_STATUS_EVALUE;

            window_params_fields.texture.width = *(int*)params->value.ptr;
        }
        else if (ARCHI_STRING_COMPARE("texture_height", ==, params->key))
        {
            if (param_texture_height_set)
                continue;
            param_texture_height_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, int))
                return ARCHI_STATUS_EVALUE;

            window_params_fields.texture.height = *(int*)params->value.ptr;
        }
        else if (ARCHI_STRING_COMPARE("window_width", ==, params->key))
        {
            if (param_window_width_set)
                continue;
            param_window_width_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, int))
                return ARCHI_STATUS_EVALUE;

            window_params_fields.window.width = *(int*)params->value.ptr;
        }
        else if (ARCHI_STRING_COMPARE("window_height", ==, params->key))
        {
            if (param_window_height_set)
                continue;
            param_window_height_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, int))
                return ARCHI_STATUS_EVALUE;

            window_params_fields.window.height = *(int*)params->value.ptr;
        }
        else if (ARCHI_STRING_COMPARE("window_flags", ==, params->key))
        {
            if (param_window_flags_set)
                continue;
            param_window_flags_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, uint32_t))
                return ARCHI_STATUS_EVALUE;

            window_params_fields.window.flags = *(uint32_t*)params->value.ptr;
        }
        else if (ARCHI_STRING_COMPARE("window_title", ==, params->key))
        {
            if (param_window_title_set)
                continue;
            param_window_title_set = true;

            if (!ARCHI_POINTER_TO_DATA_TYPE(params->value, 1, char))
                return ARCHI_STATUS_EVALUE;

            window_params_fields.window.title = params->value.ptr;
        }
        else
            return ARCHI_STATUS_EKEY;
    }

    if (param_texture_width_set)
        window_params.texture.width = window_params_fields.texture.width;

    if (param_texture_height_set)
        window_params.texture.height = window_params_fields.texture.height;

    if (param_window_width_set)
        window_params.window.width = window_params_fields.window.width;

    if (param_window_height_set)
        window_params.window.height = window_params_fields.window.height;

    if (param_window_flags_set)
        window_params.window.flags = window_params_fields.window.flags;

    if (param_window_title_set)
        window_params.window.title = window_params_fields.window.title;

    // Construct the context
    struct archi_context_sdl2_window_cpu_data *context_data = malloc(sizeof(*context_data));
    if (context_data == NULL)
        return ARCHI_STATUS_ENOMEMORY;

    archi_status_t code;
    archi_sdl2_window_cpu_context_t window =
        archi_sdl2_window_cpu_create(window_params, &code);

    if (window == NULL)
    {
        free(context_data);
        return code;
    }

    *context_data = (struct archi_context_sdl2_window_cpu_data){
        .context = {
            .ptr = window,
            .length = 1,
        },
        .texture = {
            .width = window_params.texture.width,
            .height = window_params.texture.height,
        },
    };

    *context = (archi_pointer_t*)context_data;
    return 0;
}

ARCHI_CONTEXT_FINAL_FUNC(archi_context_sdl2_window_cpu_final)
{
    struct archi_context_sdl2_window_cpu_data *context_data =
        (struct archi_context_sdl2_window_cpu_data*)context;

    archi_sdl2_window_cpu_destroy(context_data->context.ptr);
    free(context_data);
}

ARCHI_CONTEXT_GET_FUNC(archi_context_sdl2_window_cpu_get)
{
    struct archi_context_sdl2_window_cpu_data *context_data =
        (struct archi_context_sdl2_window_cpu_data*)context;

    if (ARCHI_STRING_COMPARE("window", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        *value = (archi_pointer_t){
            .ptr = archi_sdl2_window_cpu_get_window_handle(context_data->context.ptr),
            .ref_count = context->ref_count,
            .length = 1,
        };
    }
    else if (ARCHI_STRING_COMPARE("renderer", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        *value = (archi_pointer_t){
            .ptr = archi_sdl2_window_cpu_get_renderer(context_data->context.ptr),
            .ref_count = context->ref_count,
            .length = 1,
        };
    }
    else if (ARCHI_STRING_COMPARE("texture", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        *value = (archi_pointer_t){
            .ptr = archi_sdl2_window_cpu_get_texture(context_data->context.ptr),
            .ref_count = context->ref_count,
            .length = 1,
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.width", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        *value = (archi_pointer_t){
            .ptr = &context_data->texture.width,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(false, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.height", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        *value = (archi_pointer_t){
            .ptr = &context_data->texture.height,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(false, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.lock", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        archi_sdl2_pixel_t *pixels = archi_sdl2_window_cpu_get_texture_lock(
                context_data->context.ptr, NULL, NULL, NULL,
                &context_data->texture_lock.width, NULL);

        *value = (archi_pointer_t){
            .ptr = pixels,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(true, ARCHI_ALIGNOF_LOG2(archi_sdl2_pixel_t), 0),
            .length = context_data->texture_lock.width, // only first `width` pixels are contiguous
            .stride = sizeof(archi_sdl2_pixel_t),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.lock.pitch", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        archi_sdl2_window_cpu_get_texture_lock(
                context_data->context.ptr,
                &context_data->texture_lock.pitch,
                NULL, NULL, NULL, NULL);

        *value = (archi_pointer_t){
            .ptr = &context_data->texture_lock.pitch,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(true, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.lock.x", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        archi_sdl2_window_cpu_get_texture_lock(
                context_data->context.ptr, NULL,
                &context_data->texture_lock.x,
                NULL, NULL, NULL);

        *value = (archi_pointer_t){
            .ptr = &context_data->texture_lock.x,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(true, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.lock.y", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        archi_sdl2_window_cpu_get_texture_lock(
                context_data->context.ptr, NULL, NULL,
                &context_data->texture_lock.y,
                NULL, NULL);

        *value = (archi_pointer_t){
            .ptr = &context_data->texture_lock.y,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(true, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.lock.width", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        archi_sdl2_window_cpu_get_texture_lock(
                context_data->context.ptr, NULL, NULL, NULL,
                &context_data->texture_lock.width,
                NULL);

        *value = (archi_pointer_t){
            .ptr = &context_data->texture_lock.width,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(true, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else if (ARCHI_STRING_COMPARE("texture.lock.height", ==, slot.name))
    {
        if (slot.num_indices != 0)
            return ARCHI_STATUS_EMISUSE;

        archi_sdl2_window_cpu_get_texture_lock(
                context_data->context.ptr, NULL, NULL, NULL, NULL,
                &context_data->texture_lock.height);

        *value = (archi_pointer_t){
            .ptr = &context_data->texture_lock.height,
            .ref_count = context->ref_count,
            .attr = ARCHI_POINTER_DATA_ATTRIBUTES(true, ARCHI_ALIGNOF_LOG2(int), 0),
            .length = 1,
            .stride = sizeof(int),
        };
    }
    else
        return ARCHI_STATUS_EKEY;

    return 0;
}

const archi_context_interface_t archi_context_sdl2_window_cpu_interface = {
    .init_fn = archi_context_sdl2_window_cpu_init,
    .final_fn = archi_context_sdl2_window_cpu_final,
    .get_fn = archi_context_sdl2_window_cpu_get,
};


...
ARCHI_HSP_STATE_FUNCTION(archi_sdl2_hsp_state_window_cpu_render)
{
    ...
    if ((render_data == NULL) || (render_data->window == NULL) ||
    ...
    // Map texture data
    archi_memory_mapping_t texture_mapping = archi_memory_map(render_data->texture,
    ...
    {
        archi_pointer_t texture_mapping_ptr =
            archi_memory_mapping_pointer(texture_mapping);

        memcpy(texture_lock, texture_mapping_ptr.ptr,
                texture_mapping_ptr.length * texture_mapping_ptr.stride);
    }

    // Unmap texture data
    archi_memory_unmap(texture_mapping);

...
#include "archi/sdl2/hsp/window.fun.h"
#include "archi/sdl2/hsp/window.typ.h"
...
{
#define M "archi_sdl2_hsp_state_window_render"

    archi_sdl2_window_render_data_t *render_data = ARCHI_HSP_CURRENT_STATE().data;
    ...
    // Lock window texture
    code = archi_sdl2_window_lock_whole_texture(render_data->window);

    if (code != 0)
    {
        archi_log_error(M, "archi_sdl2_window_lock_whole_texture() -> %i", code);
        return;
    }

    // Get window texture lock
    archi_sdl2_pixel_t *texture_lock = archi_sdl2_window_get_texture_lock(
    ...
    {
        archi_sdl2_window_unlock_texture_and_render(render_data->window);
    ...
    code = archi_sdl2_window_unlock_texture_and_render(render_data->window);

    if (code != 0)
        archi_log_error(M, "archi_sdl2_window_unlock_texture_and_render() -> %i", code);

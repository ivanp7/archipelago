 ...
 * @brief Get current state.
 *
 * This macro utilizes the parameter `hsp` implicitly provided
 * by signature of ARCHI_HSP_STATE_FUNCTION().
 */
 ...
 * by signature of ARCHI_HSP_STATE_FUNCTION().
 */
#define ARCHI_HSP_FRAMES_ON_STACK()     archi_hsp_frames_on_stack(hsp)
 ...
 *
 * Returning from a state function normally is equivalent to calling ARCHI_HSP_RETURN(0).
 *
...
#define ARCHI_HSP_ADVANCE(num_popped_frames, ...) do {                              \
    const archi_hsp_state_t archi_hsp_advance_pushed_states[] = {__VA_ARGS__};      \
    archi_hsp_advance(hsp, (num_popped_frames),                                     \
            sizeof(archi_hsp_advance_pushed_states) / sizeof(archi_hsp_state_t),    \
            archi_hsp_advance_pushed_states);                                       \
} while (0)
 ...
 * by signature of ARCHI_HSP_STATE_FUNCTION().
 */
#define ARCHI_HSP_RETURN(num_popped_frames) do {            \
    archi_hsp_advance(hsp, (num_popped_frames), 0, NULL);   \
} while (0)

/**
 * @brief Abort hierarchical state processor execution.
 *
 * This macro utilizes the parameter `hsp` implicitly provided
 * by signature of ARCHI_HSP_STATE_FUNCTION().
 */
#define ARCHI_HSP_FINISH() do {                                         \
    archi_hsp_advance(hsp, archi_hsp_frames_on_stack(hsp), 0, NULL);    \
} while (0)

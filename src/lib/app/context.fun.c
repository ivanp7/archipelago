        ...
        archi_context_t *context)
{
    ...
    {
        if (handle == ARCHI_CONTEXT_DUMMY)
            handle = NULL;

        context->interface->final_fn(handle);
    }
...
archi_context_assign(
        ...
        const char *src_slot)
{
    if ((dest.handle == NULL) || (dest.interface == NULL))
        return ARCHI_ERROR_MISUSE;
    ...
    {
        if ((src.handle == NULL) || (src.handle == ARCHI_CONTEXT_DUMMY))
            return ARCHI_ERROR_MISUSE;
    }

...
{
    // Parse parameters
    archi_rcpointer_t default_value = {.attr = ARCHI_POINTER_ATTR__PDATA(0, char)};
    {
        archi_plist_param_t parsed[] = {
            {.name = "default_value",
                .check = {archi_value_check__attr, (archi_pointer_attr_t[]){ARCHI_POINTER_ATTR__PDATA(0, char)}},
                .assign = {archi_plist_assign__rcpointer, &default_value, sizeof(default_value), NULL}},
            {0},
        };
    ...
    ARCHI_POINTER_NULLIFY_EMPTY(default_value);
    ...
    default_value = archi_rcpointer_own(default_value, ARCHI_ERROR_PARAM);
    ...
    if (env_var != NULL)
    {
        value = (archi_rcpointer_t){
            ...
            .attr = ARCHI_POINTER_TYPE__DATA_WRITABLE |
                ARCHI_POINTER_ATTR__PDATA(strlen(env_var) + 1, char),
    ...
    if (ARCHI_STRING_COMPARE("default_value", ==, slot.name))
    {
        if (slot.num_indices != 0)
        {
            ...
            return;
                    ...
                    ARCHI_POINTER_ATTR__PDATA(0, char)))
        {
